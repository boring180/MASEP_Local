...
\begin{titlepage}
    \centering
    \vspace*{1cm}
    {\Huge\bfseries Progress Report of Marine Automatic Swarm Experiment Platform Localization System \par}
    \vspace{1.5cm}
    {\Large YH02a-25 \par}
    \vspace{0.5cm}
    ...
    \begin{enumerate}
        ...
        \item Write an RSSI-based localization program using electromagnetic transceivers on the AUVs and fixed beacons next to the tank. Calibrate the system by mapping RSSI values to distances, accounting for water attenuation and environmental factors such as salinity and temperature. Evaluate the RSSI performance in isolation through controlled experiments, comparing accuracy against vision-based results and assessing its robustness to signal interference or multipath effects in the confined tank space.
...
\section{Introduction}
    \subsection{Background and Engineering Problem } 
        ...
        By solving the problems of underwater localization, we can test marine path planning algorithms in a desk-sized platform. This may include multi-AUV cooperative object transportation~\cite{zhang_decentralized_2020}, reinforcement learning based AUV path planning~\cite{yang_intelligent_2023}. The above experiment demonstrated a solid result in simulation environments, but lacked real-world transfers. With our platform, these algorithms can be evaluated in terms of the real-to-sim gap and their potential for real-world applications. And possibly provide new solutions to the field of marine research using AUVs.
...
\section{Methodology}
    \subsection{Overview }
        ...
        \paragraph{Hardware Setup}
            The system is part of the Marine Automatic Swarm Experiment Platform (MASEP) project~\cite{cai_development_2024}, focusing on the multi-camera localization task. To achieve stable localization of AUVs, five cameras will be arranged under the water tank, with overlapping view ranges(Figure~\ref{camera-array}). All cameras will be connected to the desktop computer, which will run the localization system. With a synchronized time frame between cameras, a smooth transition between cameras can be performed. %In addition, an RSSI 
        \paragraph{Calibration}
            To establish an accurate localization, a precise calibration is required. This includes the calibration of two parts: the intrinsic parameter and the extrinsic parameter. Intrinsic calibration refers to the camera's basic parameters, including the focal length $f_x$ and $f_y$, and the focal point coordinates $c_x$ and $c_y$. To address the light refraction between different media (i.e., water and air), camera distortion parameters should also be considered~\cite{pedersen_camera_2018}. Intrinsic calibration can be validated using the reprojection error. \\
            \\
            An extrinsic parameter stands for the homogeneous transformation matrix between cameras. The rotation and translation of different cameras with respect to the world coordinate system. It is crucial to calibrate extrinsic parameters to ensure a smooth transition of localization between different cameras. In our experiment, we will use stereo camera calibration to determine the extrinsic parameters of the outer cameras relative to the center camera. To evaluate the effectiveness of the extrinsic calibration results, we adapt the reproject error calculation from intrinsic camera calibration and evaluate it between cameras.
        \paragraph{Localization}
            Localization of AUVs will be mainly based on optics localization. By detecting the ArUco markers~\cite{garrido-jurado_automatic_2014}, the algorithm can calculate the rotation and translation of the marker using PNP solvers. We moved the marker in a straight line and checked the variance of the detection, meanwhile recording the exact locations of the marker as ground truth to evaluate the localization results. Through various trials of recalibrating the camera array, we can achieve a localization error of less than $7\times 10^{-2}$m, allowing us to apply the camera array under the water tank to localize the AUVs.
        \paragraph{Sensor fusion}
            To achieve better localization of the AUVs, we will use a Kalman filter to fuse the results of different cameras. In the original system \cite{cai_development_2024}, researchers utilize a yaw-only Kalman filter \cite{li_kalman_2015} to enhance localization. In our method, we will calculate the speed of the AUVs for the prediction step and utilize the optics localization data to update the location.
            \begin{figure}
              \centering
    ...
    \subsection{Objective Statement Execution}
        ...
        \begin{itemize}
            ...
            \textbf{Expected Outcome:} The environment works, and virtual AUVs can be located by virtual cameras.\\
            \textbf{Group member in charge:} XU, Borong\\
            \textbf{Work:} The task is yet to start since it depends on the completion of task 3.
            %\begin{itemize}
            %    \item Group member in charge: XU, Borong
            %    \item Task description: Implement realistic noise models, including Gaussian noise for sensor measurements based on real-world calibration error.
            %    \item Yet to start: Depending on Task 3.
            %\end{itemize}
            ...
            %\begin{itemize}
            %    \item Group member in charge: XU, Borong
            %    \item Task description: Validate sensor fusion algorithms with the simulation environment.
            %    \item Yet to start: Depending on Task 1.2.
            %\end{itemize}
        \end{itemize}
        ...
        \begin{itemize}
            \item \textbf{Task 2.1: Chessboard photo collection}\\\\
                ...
                \textbf{Work:} We wrote a Python code to collect frames from a camera array of three, as shown in figure~\ref{collection array}. We then collect frames of a chessboard with 8×11 23mm squares covering as many camera angles as possible, while outputting chessboard detection results for validation, as shown in figure~\ref{validation}.\\\\
                When choosing a suitable chessboard, we needed one with more squares to ensure higher accuracy in the calibration steps that followed, while ensuring the squares were large enough for the cameras to detect. We also had to make sure the chessboard was able to be fully captured by all the cameras, so it could not be too large. It was a challenge to balance between these requirements. After testing with multiple chessboards, we chose the one with 8\times11 23mm squares at the end for most cases, and a 3\times16 9mm for single camera intrinsic calibration.\\\\
                All cameras will capture the synchronized frame, so different cameras can share the same object points when calibration, which will be mandatory for extrinsic calibration(Figure~\ref {chessboard}, ~\ref{multi_cameras}). The chessboard works well in air experiments, but it performs poorly underwater due to difficulties in the corner detection algorithm with complex light reflection disturbance underwater. And also, the image points can only be extracted when the whole board is detected. This shortens the range of angles when it comes to extrinsic parameter calibration. To overcome these difficulties, we developed another method to extract image points.
                \begin{figure}
                    \centering
                    \rotatebox{-90}{
            ...
            \item \textbf{Task 2.2: ChArUco photo collection}\\\\
                ...
                \textbf{Group member in charge:} XU, Borong\\
                \textbf{Work:} To overcome the poor performance of the chessboard underwater,  we use another kind of calibration board. Similar to task2.1, we wrote a Python code to collect frames of a ChArUco board with 8×11 23mm squares covering as many camera angles as possible, while outputting ChArUco detection results for validation, as shown in figure~\ref{ChArUco}.\\\\
                The ChArUco board is similar to a chessboard, while it has several ArUco marks inside the pattern. This allows the detection algorithm to extract image points without the whole board. And with this advantage in our experiment, it shows a stable detection underwater, especially for extrinsic calibration.
                \begin{figure}
                    \centering
                    \rotatebox{0}{
            ...
            %\begin{itemize}
            %    \item Group member in charge: Both members
            %    \item Task description: Perform individual camera calibration using chessboard patterns with 18×27 internal corners and 10mm square size. Determine focal length ($f_x$, $f_y$), principal point ($c_x$, $c_y$), and distortion coefficients for each of the 5 USB cameras. Validate calibration quality using reprojection error analysis.
            %    \item Partially completed: Current error $7.1\times10^{-2}m$.
            %\end{itemize}
            \begin{figure}
                    \centering
                    \rotatebox{0}{
            ...
            \textbf{Group member in charge:} Both members\\
            \textbf{Work:} We wrote an algorithm with stereo calibration techniques to determine relative camera positions and orientations between the center wide camera and the other side cameras. Then we input the collected chessboard photos again, and the algorithm outputs homogeneous transformation matrices between cameras(Equation~\ref{ext_goal}). A world coordinate system was established with the center camera as the reference frame, and reprojection error analysis was also used in this step to validate calibration quality. Recently, the error is $2.2\times10^{-2}$m, which is close to but still does not meet our expected outcome. We also roughly validated the extrinsic calibration results through visualizing the relative positions between the cameras(Figure~\ref{ext_result}).\\\\
            \begin{equation}
            \begin{aligned}
            ...
            \label{ext_goal}
            \end{equation}
            where $T$ is the optimal translation vector and $R$ is the optimal rotation matrix that minimize the Euclidean distance between corresponding 3D points $P_A$ and $P_B$ in different camera coordinate systems.

            Similar to intrinsic calibration, initial extrinsic calibration results showed higher errors due to insufficient calibration data. Increasing the number of valid captures from chessboard detection reduced the transformation error and improved calibration accuracy.
            %\begin{itemize}
            %    \item Group member in charge: Both members
            %    \item Task description: Use stereo calibration techniques to determine relative camera positions and orientations. Calculate homogeneous transformation matrices between cameras. Establish a world coordinate system with the center camera as the reference frame. Validate calibration quality using reprojection error analysis.
            %    \item Partially completed: Current error $2.2\times10^{-2}m$.
            %\end{itemize}
            \begin{figure}
                    \centering
                    \rotatebox{0}{
            ...
            \textbf{Group member in charge:} Both members\\
            \textbf{Work:} We repeated intrinsic calibration underwater to account for refraction effects at the water-air interface. By comparing calibration parameters in air and water, correction factors were developed.
            %\begin{itemize}
            %    \item Group member in charge: Both members
            %    \item Task description: Repeat intrinsic calibration underwater to account for refraction effects at the water-air interface. Compare air vs. water calibration parameters and develop correction factors.
            %    \item Partially completed: Experiments conducted in a smaller water tank, depending on Tasks 2.2 and 2.3 for further experiments.
            %\end{itemize}
        \end{itemize}
        ...
        \begin{itemize}
            ...
            \textbf{Group member in charge:} Both members\\
            \textbf{Work:} We wrote a Python code to implement real-time ArUco marker detection by OpenCV's ArUco module with DICT\_5X5\_50 dictionary. Robust localization was achieved under varying lighting conditions. When capturing chessboard photos, we also recorded some frames of an ArUco marker appearing at five fixed points and moving horizontally along a track (Figure~\ref{aruco test}) to test the algorithm. As a result, the algorithm visualized the coordinate axes of the marker and the output frame points associated with it.\\\\
            Before recording those frames, we planned the range of locations of the marker to be 2m and the distance between each fixed point to be 0.5m. However, the table in the laboratory was not large enough to allow cameras to capture the marker in such a range. Therefore, the range was narrowed to 0.5m with 0.125m distance between each fixed point. On the other hand, we did not have any rails to move the marker horizontally in a straight line, but we found that there were two straight grooves next to the table. Therefore, we put the tablet that shows the marker into the groove to solve this problem, which saved time and unnecessary expense in getting the rail for this minor task.
            \begin{figure}
                \centering
                \rotatebox{-90}{
            ...
            %\begin{itemize}
            %    \item Group member in charge: Both members
            %    \item Task description: Implement real-time ArUco marker detection using OpenCV's ArUco module with DICT\_5X5\_50 dictionary. Achieve robust localization under varying lighting conditions.
            %    \item Work completed.
            %\end{itemize}
            ...
            \textbf{Group member in charge:} XU, Borong\\
            \textbf{Work:} We wrote an algorithm to combine detections from multiple cameras. Weighted averaging of the cameras was implemented based on detection confidence and viewing angle. The algorithm also handled occlusions and marker visibility transitions between cameras.\\\\
            Currently, we can find the difference in the localization results between cameras(Figure~\ref{share_local}). And with different experiments held in water and air, we found that the image points are aligning perfectly in air, but not underwater. This implies the distortion model or the calibration process currently cannot hold the complex light environment underwater.
            %\begin{itemize}
            %    \item Group member in charge: XU, Borong
            %    \item Task description: Develop an algorithm for combining detections from multiple cameras. Implement weighted averaging based on detection confidence and viewing angle. Handle occlusions and marker visibility transitions between cameras.
            %    \item Partially completed: The error between cameras is currently too large to deploy multi-camera fusion algorithms.
            %\end{itemize}
            \begin{figure}
                    \centering
                    \rotatebox{0}{
            ...
            \begin{figure}
                    ...
                    \caption{ArUco marker trajectory tracking underwater showing increased localization variability due to refraction effects. The scattered position estimates highlight the need for refraction-corrected calibration parameters and robust multi-camera fusion algorithms.}
            ...
            %\begin{itemize}
            %    \item Group member in charge: Both members
            %    \item Task description: Localize AUVs using the ArUco tag detection algorithm in Task 3.2. Using real-world measurements on the AUV location and comparing with the detection algorithm. Localize at least 3 AUVs with positional accuracy within $\pm 7\times 10^ {- 2} $ m and achieve a detection speed of at least 10Hz.
            %    \item Yet to start: Depending on Tasks 2.4 and 3.2.
            %\end{itemize}
        \end{itemize}
        
        \subsubsection{Objective Statement 4: Sensor Fusion Implementation}
        The objective is to utilize a sensor fusion algorithm to combine the results from different cameras.
        
        \textbf{Tasks:}
        \begin{itemize}
            \item \textbf{Task 4.1: Extended Kalman Filter Implementation}\\\\
            \textbf{Aim:} Implement the Extended Kalman Filter to combine the results of different cameras.\\
...
\section{Project Plan}
    \subsection{Project Timeline Analysis}

        \textbf{Task Dependencies and Critical Path:} The project follows a sequential workflow where calibration tasks (Priority 1) must precede tracking implementation (Priority 2), which enables sensor fusion development (Priority 3). The critical path includes: Camera calibration → ArUco tracking → Multi-camera fusion → Sensor integration. Any delays in calibration will cascade through subsequent objectives, potentially extending the project timeline by 2-3 weeks.

        \textbf{Risk Assessment and Mitigation Strategies:}
        \begin{itemize}
            \item \textbf{Hardware Failure Risk:} Camera malfunctions or synchronization issues could delay data collection. \textit{Mitigation:} Maintain backup cameras and implement software-based frame alignment as contingency.
            \item \textbf{Calibration Accuracy Risk:} Underwater refraction effects may prevent achieving target precision. \textit{Mitigation:} Develop air-water calibration correction models and implement iterative refinement procedures.
            \item \textbf{Algorithm Complexity Risk:} Multi-camera fusion algorithms may require extensive tuning. \textit{Mitigation:} Start with single-camera validation before multi-camera implementation, with fallback to single-camera mode.
            \item \textbf{Resource Availability Risk:} Limited access to MASEP platform during peak academic periods. \textit{Mitigation:} Schedule intensive testing phases during project milestones and maintain simulation environments for offline development.
        \end{itemize}

        \textbf{Labor Allocation:} Total estimated effort is 480 hours across 17 weeks. XU, Borong focuses on algorithmic development (60\% of effort), while CHAN, Sze Sen handles hardware integration and data collection (40\% of effort). Weekly meetings ensure coordinated progress and early identification of bottlenecks.

        \textbf{Contingency Planning:} A 2-week buffer is allocated for unexpected delays. Alternative calibration methods (e.g., manual point measurement) and simplified tracking algorithms serve as fallback options if primary approaches encounter difficulties.
...
\addcontentsline{toc}{section}{Appendices}
    ...
    \addcontentsline{toc}{subsection}{Meeting minutes}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \item Prof. YIN also suggested several papers that could be referred to in the proposal report
        \end{itemize}
    \end{itemize}
    \begin{longtable}[c]{| l | l | l | l |}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \item Prof. YIN provided feedback on refining the calibration approach,  suggested additional references for modeling cross-medium distortion, and also suggested implying RSSI localization as an assisting method
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \item Prof. YIN provided feedback on addressing the calibration gap through additional modeling and suggested references for RSSI implementation and Python environments
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \item Prof. YIN mentioned that he would leave HKUST, so the supervisor of this project would be changed to Prof. Fumin ZHANG
            \item We plan to continue the lab experiment after finishing the progress report and midterm poster
            \item Prof. YIN suggested that we should include more about the gaps of traditional localization methods and our plan for the remaining parts of this project in the progress report, and add more images instead of much text to draw attention in the midterm poster
        \end{itemize}
    \end{itemize}
    ...
    \end{itemize}
    \newpage
    \subsection*{Appendix B-Group Members’ Contribution}
    \addcontentsline{toc}{subsection}{Group Members’ Contribution}
    ...
    \newpage
    \subsection*{Appendix C-Monthly Reports}
    \addcontentsline{toc}{subsection}{Monthly Reports}
    \begin{figure}
        \centering
