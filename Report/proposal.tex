...
\section{Methodology}
    \subsection{Overview }
        ...
        \paragraph{Localization}
            Localization of AUVs will be mainly based on optics localization. By detecting the ArUco markers~\cite{garrido-jurado_automatic_2014}, the algorithm can calculate the rotation and translation of the marker using PnP solvers. We moved the marker in a straight line and checked the variance of the detection, meanwhile recording the exact locations of the marker as ground truth to evaluate the localization results. RSSI localization acts as an assistance. By receiving the signal strength from the AUVs and more than 4 other devices emitting Wi-Fi signals using a signal sensor, the algorithm can estimate the coordinates of the AUVs by matching the results of fingerprinting. Through various trials of recalibrating the camera array and RSSI sensor, we can achieve a localization error of less than $7\times 10^{-2}$m, allowing us to apply the camera array under the water tank and the sensor at a corner of the top of the water tank to localize the AUVs.
    ...
    \subsection{Objective Statement Execution}
        ...
        \begin{itemize}
            ...
            \textbf{Expected Outcome:} The environment works, and virtual AUVs can be located by virtual cameras.\\
            \textbf{Group member in charge:} XU, Borong\\
            \textbf{Work:} We model of the camera projection error as a gamma distribution $f(x)=\frac{1}{\Gamma(\alpha)\theta^\alpha}x^{\alpha-1}e^{\frac{-x}{\theta}}$ in pixel level. With the probability distribution of the per-pixel error, we can model the effect of noise on the estimation and run the simulation to tune the parameter of the extended Kalman Filters.(Figure~\ref{error_distribution}
            \begin{figure}
                    \centering
                    \rotatebox{0}{
            ...
            \textbf{Group member in charge:} XU, Borong\\
            \textbf{Work:} We use Kalman filter to fuse the camera localization results in the simulation environment. Reaching a $0.3\times10^{-2}$m error, which is in the range of our goal. Although the simulation environment cannot fully model the occultation and other noise, but it shows the effectiveness of using Kalman Filter to improve localization accuracy(Figure~\ref {EKF}).
            \begin{figure}
                    \centering
                    \rotatebox{0}{
        ...
        \begin{itemize}
            ...
            \textbf{Group member in charge:} XU, Borong\\\
            \textbf{Work:} We wrote an algorithm with stereo calibration techniques to determine relative camera positions and orientations between the center wide camera and the other side cameras. Then we input the collected chessboard photos again, and the algorithm outputs homogeneous transformation matrices between cameras. A world coordinate system was established with the center camera as the reference frame, and reprojection error analysis was also used in this step to validate calibration quality.We also roughly validated the extrinsic calibration results through visualizing the relative positions between the cameras(Figure~\ref{ext_result}), and it matches with our measurements of specification of the camera array.\\\\
            \begin{equation}
            \begin{aligned}
            ...
            \end{figure}
            In the under water settings. Since the field of view of the camera setting is not enough to capture the whole chess board in two camera frames, the traditional extrinsic calibration method using chessboard is not enough in our underwater environment. We propose a new method of using ChArUco board to calibration the camera matrix extrinsic parameters(Figure~\ref{charuco_capture}). Which utilize PnP solver to find both camera's relative transformation to the ChArUco calibration board, and produce pairs of camera location. With the camera point correspondence, we can solve the transformation between cameras(Equation~\ref{ext_goal}). Where $T$ is the optimal translation vector and $R$ is the optimal rotation matrix that minimize the Euclidean distance between corresponding 3D points $P_A$ and $P_B$ in different camera coordinate systems.
            ...
            \textbf{Group member in charge:} XU, Borong\\\
            \textbf{Work:} We repeated intrinsic calibration underwater and discovered the calibration results failed in underwater environments. This could due to the light refraction when crossing different medium. \\
            To test the effectiveness of the intrinsic calibration result, we use the extrinsic calibration results to compare the localization between different cameras. Since the extrinsic parameters consist of transformations between cameras, this will not be effect by light refraction. Once the extrinsic parameters is confirmed effective in air, we can use it to validate the intrinsic parameters produced by out method.\\
            The cross medium light refraction invalidates the pinhole camera assumption and the traditional polynomial distortion model is not enough to estimate the distortion. Therefore, we propose our new method for underwater distortion calibration.\\\\
            Our new method calibrates the camera parameter $K$ and distortion model $D$ separately. And we model the distortion parameter $D$ as a vector field We first apply OpenCV calibration program with assumption of no distortion to get the intrinsic parameter $K$, and using this $K$ matrix, we normalize the objects points using opencv undistort re-projected image points as a ground truth for distortion parameters. Then, we calibrate the distortion parameters using gradient descent with PyTorch. Using the new distortion model, we undistort the image points and calibrate the intrinsic parameter $K$ again.\\\
            \begin{algorithm}
            ...
            For the model fo the vector field, we apply a traditional polynomial distortion with radial distortion and tangential distortion(Equation~\ref{polynomial}). And also a Multi-Layer-Perceptron model with tanH activation function to model the nonlinear relationship. The results can be found in (Figure~\ref{int_results}).  
        ...
        \begin{itemize}
            ...
            \textbf{Group member in charge:} XU, Borong\\
            \textbf{Work:} We wrote an algorithm to combine detections from multiple cameras. Weighted averaging of the cameras was implemented based on detection confidence and viewing angle. The algorithm also handled occlusions and marker visibility transitions between cameras by estimating the AUV's position from previous frames.\\\\
